...
def plot_efficient_frontier(mu, cov, tickers, vols, rets, portfolios=None, n_random=3000, title_suffix=""):
    ...
    sc = ax.scatter(np.array(rand_vols)*100, np.array(rand_rets)*100, c=rand_sharpes, cmap='viridis', s=6, alpha=0.4)
    ...
    for i, ticker in enumerate(tickers):
        ...
        ax.scatter(asset_vol, mu[i]*100, marker='D', s=70, edgecolor='black', linewidth=0.8, zorder=5)
        ax.annotate(ticker, (asset_vol, mu[i]*100), xytext=(6, 6), textcoords='offset points', fontsize=10)

    if portfolios:
        for label, (w, colour, marker) in portfolios.items():
            r, v, s = portfolio_stats(w, mu, cov)
            ax.scatter(v*100, r*100, marker=marker, s=350, color=colour, edgecolor='black', zorder=6, label=f'{label} ({s:.3f})')
