...
def _fetch_complete_close_series(fetch_fn, ticker, required_dates, period, max_attempts=3, retry_delay_seconds=1.0):
    last_error = None
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if df.empty or 'Close' not in df.columns:
                raise ValueError("no closing prices were returned")

            close = df['Close'].copy()
            if not isinstance(close, pd.Series):
                raise ValueError("the response did not contain one closing-price series")

            #daily bars are labelled by their exchange-session date
            close.index = pd.DatetimeIndex(close.index).tz_localize(None).normalize()
            if close.index.hasnans or close.index.has_duplicates:
                raise ValueError("price dates were invalid or duplicated")

            close = pd.to_numeric(close, errors='raise').reindex(required_dates)
            values = close.to_numpy(dtype=float)
            invalid_dates = close.index[~np.isfinite(values) | (values <= 0)]

            if len(invalid_dates):
                shown_dates = ', '.join(date.strftime('%Y-%m-%d') for date in invalid_dates[:5])
                if len(invalid_dates) > 5:
                    shown_dates += ', ...'
                raise ValueError(f"missing/invalid required sessions ({len(invalid_dates)}): {shown_dates}")
    ...
    raise RuntimeError(f"Could not obtain complete closing-price history for {ticker} after {max_attempts} attempts; {last_error}") from last_error
...
def generate_trading_decisions(tickers, signal_date, trading_dates, top_frac=0.20, rebalance_days=21, lookback_days=63, state_path=None, period="1y"):
    ...
    for ticker in tickers:
        price_data[ticker] = _fetch_complete_close_series(fetch_fn=fetch_price_data, ticker=ticker, required_dates=required_dates, period=period)
...
def build_rebalance_preview(client, tickers, env_path, state_path, lookback_days=63, top_frac=0.20, rebalance_days=21, execution_window_minutes=15, price_max_age_seconds=60, cash_reserve_fraction=0.01):
    ...
    broker_time = broker_time.tz_convert('America/New_York')
    calendar_gate = bool(clock.is_open and execution_open <= broker_time < execution_end)
    result['calendar_gate'] = calendar_gate
