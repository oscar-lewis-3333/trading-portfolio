...
def _portfolio_value(state):
    if not isinstance(state, dict):
        return None

    snapshot = state.get('account_snapshot')
...
def format_run_alert(result):
    ...
    if isinstance(pending, dict):
        ...
        if status == 'completed':
            current_value = _portfolio_value(pending.get('completed_state'))

            if current_value is not None:
                lines.append(f"Portfolio value: ${current_value:,.2f}")
                previous_value = _portfolio_value(pending.get('previous_state'))

                if previous_value is None:
                    lines.append("Change since previous completed rebalance: N/A (first recorded baseline)")
                else:
                    percentage_change = (current_value / previous_value - 1) * 100
                    lines.append(f"Change since previous completed rebalance: {percentage_change:+.2f}%")
