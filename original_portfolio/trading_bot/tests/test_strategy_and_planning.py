...
class StrategyAndPlanningTests(unittest.TestCase):
    ...
    def test_decision_history_retries_one_incomplete_ticker(self):
        tickers = [f'T{i:02d}' for i in range(5)]
        sessions = pd.bdate_range('2024-01-02', periods=64)
        calls = {ticker: 0 for ticker in tickers}
        module = types.ModuleType('data_loader')

        def fetch_price_data(ticker, period='1y'):
            calls[ticker] += 1
            frame = pd.DataFrame({'Close': np.linspace(100.0, 110.0 + tickers.index(ticker), len(sessions))}, index=sessions)
            if ticker == 'T02' and calls[ticker] == 1:
                frame = frame.drop(sessions[1])
            return frame
        ...
        with tempfile.TemporaryDirectory() as directory:
            state_path = Path(directory) / 'basket_state.json'
            with patch.dict(sys.modules, {'data_loader': module}), patch('pipeline.time.sleep'):
                decisions, plan = generate_trading_decisions(tickers, sessions[-1], sessions, top_frac=0.4, rebalance_days=21, lookback_days=63, state_path=state_path)

        self.assertTrue(plan['rebalance_needed'])
        self.assertEqual(len(decisions), 2)
        self.assertEqual(calls['T02'], 2)
        self.assertTrue(all(calls[ticker] == 1 for ticker in tickers if ticker != 'T02'))

    def test_decision_history_failure_names_ticker_date_and_attempts(self):
        tickers = [f'T{i:02d}' for i in range(5)]
        sessions = pd.bdate_range('2024-01-02', periods=64)
        calls = {ticker: 0 for ticker in tickers}
        module = types.ModuleType('data_loader')
        ...
        with tempfile.TemporaryDirectory() as directory:
            state_path = Path(directory) / 'basket_state.json'
            expected_date = sessions[1].strftime('%Y-%m-%d')
            with patch.dict(sys.modules, {'data_loader': module}), patch('pipeline.time.sleep'):
                with self.assertRaisesRegex(RuntimeError, rf'T02 after 3 attempts;.*{expected_date}'):
                    generate_trading_decisions(tickers, sessions[-1], sessions, top_frac=0.4, rebalance_days=21, lookback_days=63, state_path=state_path)

        self.assertEqual(calls['T02'], 3)
