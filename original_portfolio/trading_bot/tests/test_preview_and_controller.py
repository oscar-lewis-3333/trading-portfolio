...
class PreviewAndControllerTests(unittest.TestCase):
    ...
    def test_preview_refreshes_clock_after_quote_fetch(self):
        tickers = [f'T{i:02d}' for i in range(20)]
        selected = tickers[:4]
        decisions = pd.DataFrame({'ticker': selected, 'target_weight': [0.25] * 4})
        events = []
        clocks = iter([
            SimpleNamespace(timestamp=pd.Timestamp('2024-06-04 09:35:00', tz='America/New_York'), is_open=True),
            SimpleNamespace(timestamp=pd.Timestamp('2024-06-04 09:35:06', tz='America/New_York'), is_open=True),
        ])
        client = PreviewClient()
        ...
        quote_client = QuoteClient(selected)

        def get_quotes(request):
            events.append('quotes')
            return {ticker: SimpleNamespace(bid_price=99.0, ask_price=100.0, timestamp=pd.Timestamp('2024-06-04 09:35:05', tz='America/New_York')) for ticker in selected}

        quote_client.get_stock_latest_quote = get_quotes
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch('pipeline.generate_trading_decisions', return_value=(decisions, plan)), patch('broker.get_data_client', return_value=quote_client):
                preview = build_rebalance_preview(client, tickers, env_path=Path(directory) / '.env', state_path=state_path)

        self.assertEqual(preview['status'], 'ready_to_prepare')
        self.assertEqual(preview['price_ages_seconds'], {ticker: 1.0 for ticker in selected})
