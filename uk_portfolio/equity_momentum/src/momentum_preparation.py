...
def apply_verified_price_repairs(prices, repairs):
    """Restore explicitly reviewed missing rows, never infer execution quotes."""
    out = prices.copy()
    out["verified_price_repair"] = False
    seen = set()
    for repair in repairs:
        key = (pd.Timestamp(repair["date"]), repair["ticker"])
        if key in seen or key not in out.index:
            raise ValueError(f"Duplicate or absent repair key: {key}")
        seen.add(key)
        row = out.loc[key]
        if row[PRICE_COLUMNS].notna().any():
            raise ValueError(f"Repair would overwrite an existing quote: {key}")
        if any(bool(row.get(flag, False)) for flag in BOOLEAN_COLUMNS[1:]):
            raise ValueError(f"Cannot repair a blocked quote: {key}")
        if (row[["Dividends", "Stock Splits"]].isna().any()
            or row[["Volume", "Dividends", "Stock Splits"]].astype(float).fillna(0).ne(0).any()):
            raise ValueError(f"Repair conflicts with recorded activity or actions: {key}")

        currency = repair["quote_currency"]
        if currency not in ("GBP", "GBX", "GBp"):
            raise ValueError("Verified repairs require GBP or pence quotes.")
        divisor = 1. if currency == "GBP" else 100.
        quotes = pd.Series(repair["quotes"], dtype=float)
        ohlc = quotes.reindex(["Open", "High", "Low", "Close"]) / divisor
        volume = quotes.get("Volume", np.nan)
        factor = float(repair["adjustment_factor"])
        if (not np.isfinite(ohlc).all() or ohlc.le(0).any()
            or not np.isfinite(volume) or volume <= 0 or volume % 1
            or not np.isfinite(factor) or factor <= 0
            or ohlc.High < ohlc.max() or ohlc.Low > ohlc.min()):
            raise ValueError(f"Invalid verified OHLCV or adjustment factor: {key}")

        history = out.xs(key[1], level="ticker")
        position = history.index.get_loc(key[0])
        if position == 0 or position == len(history) - 1:
            raise ValueError("Repair needs adjacent observations to check its share basis.")
        neighbours = history.iloc[[position - 1, position + 1]]
        if (not np.allclose(neighbours.adjustment_factor, factor, rtol=1e-10, atol=0)
            or neighbours[["Dividends", "Stock Splits"]].isna().any().any()
            or neighbours[["Dividends", "Stock Splits"]].ne(0).any().any()):
            raise ValueError(f"Repair adjustment basis is not established: {key}")

        out.loc[key, ["Open", "High", "Low", "Close"]] = ohlc.to_numpy()
        out.loc[key, "Adj Close"] = ohlc.Close * factor
        out.loc[key, ADJUSTED_COLUMNS] = ohlc.to_numpy() * factor
        out.loc[key, "adjustment_factor"] = factor
        out.loc[key, "Volume"] = volume
        out.loc[key, ["Dividends", "Stock Splits"]] = 0.
        out.loc[key, ["observed_row", "verified_price_repair"]] = True
...
def load_prepared_momentum_data(project_dir=None):
    ...
    logic_hashes = manifest.get("python_logic_sha256", {})
    for relative, expected in manifest["local_sha256"].items():
        if relative in logic_hashes:
            if python_logic_sha256(project / relative) != logic_hashes[relative]:
                raise ValueError(f"Preparation logic changed: {relative}; review before rebuilding.")
        elif sha256(project / relative) != expected:
...
def prepare_momentum_data(project_dir=None, *, rebuild=False):
    ...
    repairs_path = project / "data/verified_price_repairs_v1.json"
    ...
    repair_records = json.loads(repairs_path.read_text())["repairs"]
    evidence_paths = []
    for repair in repair_records:
        for evidence in repair["evidence"]:
            path = project / evidence["path"]
            if sha256(path) != evidence["sha256"]:
                raise ValueError(f"Price-repair evidence changed: {path}")
            evidence_paths.append(path)
    prices = apply_verified_price_repairs(prices, repair_records)
    validate_price_panel(prices, schedule)
    ...
    limitations = ["Present-day candidate catalogue; historical broker membership and delisted-stock coverage are incomplete.",
        ...
        "Explicit missing-row repairs use archived secondary-source quotes; these are daily references, not verified executable auction prices.",
    ...
    local_paths = [universe_path, review_path, repairs_path,
                   project / "src/momentum_data.py", Path(__file__)] + evidence_paths
    ...
    manifest = {"built_utc": pd.Timestamp.now(tz="UTC").isoformat(),
        ...
        "python_logic_sha256": {
            str(p.relative_to(project)): python_logic_sha256(p)
            for p in local_paths if p.suffix == ".py"
        },
        ...
        "summary": {"candidate_tickers": len(universe), "downloaded_tickers": len(inherited_symbols) + len(downloads),
            ...
            "verified_price_repairs": int(prepared.verified_price_repair.sum()),
    ...
    result = dict(prices=prepared, schedule=schedule, universe=universe, coverage=coverage,
                  ...
                  verified_price_repairs=repair_records,
